...
<?php
    ...
    if(isset($_GET['ID'])){
        ...
        if($ID===""){
            ...
            $oneClass=getOneClass($ID);
            $row = $oneClass->fetch_assoc();
            // echo var_dump($row);
        }
    }
?>
...
<body>
    ...
    <div class="creatClass">
        <div class="container">
            <form action="./updateClass.php?ID=<?=$row['ID']?>" method="post">
                <div class="col-50">
                    <h3>Sửa Lớp Học</h3>

                    <label for="className">Tên lớp học (bắt buộc)</label>
                    <input type="text" id="className" name="className" required value="<?=$row['ClassName'] ?>">
                    <label for="subject">Môn học</label>
                    <input type="text" id="subject" name="subject" required value="<?=$row['Subjects'] ?>">
                    <label for="classMate">Phòng học</label>
                    <input type="text" id="classMate" name="classMate" required value="<?=$row['ClassMate'] ?>">
                    <label for="classImage">Class Image</label>
                    <input type="file" id="classImage" name="classImage" required value="<?=$row['Images'] ?>">
